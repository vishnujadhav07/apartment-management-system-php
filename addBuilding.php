...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = "SELECT * FROM building WHERE id='$bid' and name='$bname' and address='$baddress'";
  $result = mysqli_query($db, $sql);
  $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
  $count = mysqli_num_rows($result);
  if ($count == 0) {
    $sql = "INSERT into building (id,name, address) VALUES ( '$bid','$bname', '$baddress')";
    $result = mysqli_query($db, $sql);
...
<body>
  ...
  <div class="content container" style="text-align:center;">
    <!--nav bar-->
    <br> <br>
    <p style="font-size: 20px; font-weight: 500; color: darkblue;"> <h2>Add Building </h2></p>
    <br>

    <form method="post" class="form-add-building">
    <div class="row mb-3">
        <label for="bid" class="col-sm-3 col-form-label">Building ID</label>
        <div class="col-sm-9">
          <input type="int" class="form-control" id="bid" name="bid" required>
        </div>
      </div>
      <div class="row mb-3">
  ...
  <footer class="footer footer-distributed sticky-bottom">
    ...
    <div class="footer-right">
      ...
      <a href="mailto:rvce.edu.in"><i class="fas fa-envelope"></i></a>
